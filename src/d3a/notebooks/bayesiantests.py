...
def plot_posterior(ax, points, p, names=('C1', 'C2'), triangle_color='orange', points_cmap=plt.cm.Blues_r, linewidth=1):
    ...
    '''
    # triangle filling
    ax.fill([0, 0.5, 1.0, 0], [0, np.sqrt(3) / 2, 0, 0], color="gainsboro")
    ...
    ax.text(-0.0, -0.12, f'p({names[0]})\n({pr})', color="black", horizontalalignment='center')
    ax.text(0.50, np.sqrt(3) / 2 + 0.05, f'p(rope)\n({pe})', color="black",  horizontalalignment='center')
    ax.text(0.99, -0.12, f'p({names[1]})\n({pl})', color="black",  horizontalalignment='center')

    # project and draw points
    tripts = _project(points[:, [0, 2, 1]])
    ax.hexbin(tripts[:, 0], tripts[:, 1], mincnt=1, cmap=points_cmap)            
