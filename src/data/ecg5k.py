...
def load_data(return_test=True, return_train=True):
    if return_train:
        X_train, Y_train = load_file('../data/ecg5000/ECG5000_TRAIN.txt')
        scaler = StandardScaler()
        X_train[:, 0] = scaler.fit_transform(X_train[:, 0]).round(3)
        X_train = (X_train, Y_train)
        
    if return_test:
        X_test, Y_test = load_file('../data/ecg5000/ECG5000_TEST.txt')
        scaler = StandardScaler()
...
def load_generators(return_test=True, return_train=True, tslen=140):
    ...
    if return_train:
        ...
        gen_train = ECG5KSequence(X_train, 
                                extra_channel=False, batch_size=32, 
                                batches_per_epoch=15, ts_len=tslen) 
    ...
    if return_test:
        ...
        gen_test = ECG5KSequence(X_test, 
                                extra_channel=False, batch_size=32, 
                                batches_per_epoch=120, ts_len=tslen) 
        del X_test
        gc.collect()
...
class ECG5KSequence(tf.keras.utils.Sequence):
    """
    """
    def __init__(self, X, ts_len=140, batch_size=32, 
